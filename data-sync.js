  ...
  try{
    ...
    for(const [id,legacy] of legacyById){
      ...
      const comparisons={
        ...
        dueDate:{legacy:String(Object.prototype.hasOwnProperty.call(legacy,'deadline')?(legacy.deadline||''):(legacy.date||legacy.dueDate||'')),d1:String(d1.canonicalDueDate??d1.date??''),different:canonicalDate(Object.prototype.hasOwnProperty.call(legacy,'deadline')?(legacy.deadline||''):(legacy.date||legacy.dueDate||''))!==canonicalDate(d1.canonicalDueDate??d1.date)},
