...
function renderPplList(filter){
  ...
  el.innerHTML=list.map(p=>{
    ...
    const pe=p.tasks.filter(isOpenTask).length;
...
function selectPpl(name){
  ...
  const ptActive=pt.filter(t=>nstt(t.status)!=="Cancelled").length;
  const pct=ptActive?Math.round(dn/ptActive*100):0;
  document.getElementById("ppl-right").innerHTML=`
    <div style="display:flex;align-items:center;gap:14px;margin-bottom:18px;padding-bottom:16px;border-bottom:1px solid var(--border)">
      ${av(name,44)}<div style="flex:1">
        ...
        <div style="display:flex;gap:12px;margin-top:5px"><span style="font-size:12px;font-weight:600;color:#15803d">${dn} completed</span><span style="font-size:12px;font-weight:600;color:${pt.filter(isOverdueTask).length?"#b91c1c":"var(--muted)"}">${pt.filter(isOpenTask).length} open</span></div>
      </div>
...
function renderDeptList(){
  ...
  document.getElementById("dept-list").innerHTML=Object.entries(dp).map(([d,dt])=>{
    const active=dt.filter(t=>nstt(t.status)!=="Cancelled");
    const dn=active.filter(t=>nstt(t.status)==="Done").length;
    const ov=active.filter(isOverdueTask).length;
    ...
    return `<div class="dept-row${curDeptSel===d?" sel":""}" onclick="selectDept('${d}')">
      ...
      <div><div class="dr-nm">${d}</div><div class="dr-cnt">${active.length} tasks &bull; ${dn} done${ov?` &bull; <span style="color:#b91c1c">${ov} overdue</span>`:""}</div></div>
    </div>`;
  }).join("");
}
function selectDept(dept){
  ...
  const dtActive=dt.filter(t=>nstt(t.status)!=="Cancelled");
  const dn=dtActive.filter(t=>nstt(t.status)==="Done").length;
  const pct=dtActive.length?Math.round(dn/dtActive.length*100):0;
  document.getElementById("dept-right").innerHTML=`
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:18px;padding-bottom:16px;border-bottom:1px solid var(--border)">
      ...
      <div style="flex:1"><div style="font-size:17px;font-weight:700;color:var(--body);margin-bottom:1px">${dept}</div><div style="font-size:12px;color:var(--muted)">${dtActive.length} tasks &bull; ${pct}% complete</div></div>
      <div style="text-align:right"><div style="font-size:22px;font-weight:700;color:var(--body)">${pct}%</div><div class="prog-track" style="width:72px;margin-top:4px"><div class="prog-fill" style="width:${pct}%"></div></div></div>
    </div>
